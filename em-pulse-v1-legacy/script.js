document.addEventListener('DOMContentLoaded', () => {
  ...
  function cargarSeccion(seccion) {
    // const url = seccion === 'testimonios' ? '/EM-PULSE/testimonios.php' : `${seccion}.html`;
    const url = seccion === 'testimonios' ? '/EM-PULSE/componentes/testimonios.php' : `${seccion}.html`;
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        // eventos dentro de contenido dinámico
        bindHamburger();
        bindFormValidation();
        if (seccion === 'noticias') fetchNoticias();
      })
      .catch(error => {
        contentContainer.innerHTML = `<p style="color:red;">❌ Error al cargar ${seccion}</p>`;
        console.error(error);
      });
  }
